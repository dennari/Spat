...
contours=FALSE,labels=FALSE,...)
{
    ...
    if (TRUE) {
       tmp <-levelplot(row.values=mapgrid$x,column.values=yy,x=plotdata,
                        mm=list(x=map$x,y=map$y),
                        panel=levelplotmap,
                        col.regions=color.palette,
                        xlim=range(mapgrid$x),ylim=range(yy),aspect="iso",
                        contour=contours,labels=labels,pretty=TRUE,
...
globemap = read.table("globe_map.txt")

globemapflat = (list(x = atan2(globemap[,2],globemap[,1]),y = globemap[,3]))
globemap.length = length(globemapflat$x)
iii = which(globemapflat$x[-globemap.length]*globemapflat$x[-1] < 0)
while (length(iii)>0) {
    globemapflat = (list(x = (c(globemapflat$x[1:iii[1]],NA,
                                globemapflat$x[(iii[1]+1):globemap.length])),
                         y = (c(globemapflat$y[1:iii[1]],NA,
                                globemapflat$y[(iii[1]+1):globemap.length])) ))
    iii = which(globemapflat$x[-globemap.length]*globemapflat$x[-1] < 0)
    globemap.length = length(globemapflat$x)
}
